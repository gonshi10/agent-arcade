...
const { findGitRoot, ensureGitignorePatterns } = require("./gitignore");

const MARKER = "#agent-arcade";
const EVENTS = ["UserPromptSubmit", "PreToolUse", "Notification", "Stop"];

// Absolute path to our CLI, resolved from this package — works regardless of how
// the package is installed in the consuming repo (no `npx` latency on a hook).
 ...
 * autostart=true (default): UserPromptSubmit ensures the server is up (starting
 *   it + opening the game on a cold start) before posting `working`, so a prompt
 *   auto-launches the game with no manual `start`. autostart=false keeps the old
 *   bare-curl behavior (assumes the server is already running).
 */
function buildHooks(port, { precise = false, autostart = true } = {}) {
  ...
  // Self-bootstrapping prompt hook: ensure server up + open game once + post.
  const workingCmd = autostart
    ? `node "${BIN}" hook working --port ${port} >/dev/null 2>&1 || true ${MARKER}`
    : c("working");
  ...
  return {
    UserPromptSubmit: [{ hooks: [{ type: "command", command: workingCmd }] }],
...
function gitignorePatternsForScope(scope) {
  if (scope === "global") return [];
  if (scope === "shared") return [".claude/settings.json.bak"];
  return [".claude/settings.local.json", ".claude/settings.local.json.bak"];
}
...
function install({
  ...
  const ours = buildHooks(port, { precise, autostart });
  ...
  const patterns = gitignorePatternsForScope(scope);
  if (gitignore && patterns.length) {
    const root = findGitRoot(cwd);
    if (root) gitignoreResult = ensureGitignorePatterns(root, patterns);
  }

  return { file, backedUp, precise, autostart, gitignore: gitignoreResult };
}
